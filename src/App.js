...
function App() {
  ...
  const loadBlockchainData = async () => {
    ...
    if (chainId === 4) {
      ...
      setPrice(price);
      const MAX_SUPPlY = await contract.methods.PRESALE_RESERVED().call();
  ...
  return (
    <>
      ...
      <InformationModal
        open={preSale}
        onClose={setPreSale}
        title="Presale Not Open"
        text="Presale not open yet. Follow us on Discord and Twitter fo the updates"
      />
      <InformationModal
        open={saleLive}
        onClose={setSaleLive}
        title="Presale / Sale is not live"
        text="Presale / Sale is not live yet. Please follow our discord for the updates"
      />
      <InformationModal
